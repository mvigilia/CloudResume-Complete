	...
	// Main.
		...
		const counter = document.querySelector(".views");
			async function updateCounter(){
				let response = await fetch("https://hqhueg2gqo36zewlqeafa2u2640rebjo.lambda-url.ap-southeast-2.on.aws/")
...
document.getElementById('contactForm').addEventListener('submit', async function (event) {
	...
	try {
	  ...
	  if (response.ok) {
		...
		// If response isn't OK, parse and display error message
		const errorResult = await response.json();
		alert(errorResult.message || 'There was an error submitting your form.');
	  }
